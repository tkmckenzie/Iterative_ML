...
for (k.restricted in 1:(k-1)){
  load(sprintf("stan_par_translog_fits/stan_par_sf_restr_beta_partial_%i.RData", k.restricted))
  stan.extract = extract(stan.fit)
...
load("stan_par_translog_fits/stan_par_sf_restr_beta.RData")
stan.extract = extract(stan.fit)
traceplot(stan.fit)
readline("Press Enter to Continue.")
...
#beta and beta_const
load("stan_par_translog_fits/stan_par_sf_restr_beta_beta_const.RData")
stan.extract = extract(stan.fit)
traceplot(stan.fit)
readline("Press Enter to Continue.")
...
load("stan_par_translog_fits/stan_par_sf_restr_beta_beta_const_sigma_v.RData")
stan.extract = extract(stan.fit)
traceplot(stan.fit)
readline("Press Enter to Continue.")
