...
#sigma.u
load("stan_par_translog_fits/stan_par_sf_restr_sigma_u.RData")
stan.extract = extract(stan.fit)
if (show.trace){
  show(traceplot(stan.fit))
  readline("Press Enter to Continue.")
...
#sigma.u, sigma.v
load("stan_par_translog_fits/stan_par_sf_restr_sigma_uv.RData")
stan.extract = extract(stan.fit)
if (show.trace){
  show(traceplot(stan.fit))
  readline("Press Enter to Continue.")
...
for (k.restricted in 1:(k-1)){
  load(sprintf("stan_par_translog_fits/stan_par_sf_restr_sigma_uv_beta_partial_%i.RData", k.restricted))
  stan.extract = extract(stan.fit)
  if (show.trace){
    show(traceplot(stan.fit))
    readline("Press Enter to Continue.")
...
load("stan_par_translog_fits/stan_par_sf_restr_sigma_uv_beta.RData")
stan.extract = extract(stan.fit)
if (show.trace){
  show(traceplot(stan.fit))
  readline("Press Enter to Continue.")
