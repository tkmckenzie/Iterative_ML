...
for (run in 1:(num.runs - 1)){
  ...
  if (num.free.params == 1){
    stan.file = "stan_model_iterative_singlefree.stan"
    load("restricted.singlefree.stan.dso.RData")
  } else{
    stan.file = "stan_model_iterative.stan"
    load("restricted.stan.dso.RData")
  }
