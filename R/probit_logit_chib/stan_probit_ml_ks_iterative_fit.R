...
#Unrestricted model
if (ncol(X) > 1){
  load("probit.unrestricted.stan.dso.RData")
  
  model.data = list(N = N, k = ncol(X),
                    y = c(y), X = X,
                    mu_beta = mu.beta, sd_beta = sd.beta)
  stan.fit = stan("stan_probit_model.stan",
                  data = model.data,
                  chains = 1,
  ...
  # save(unrestricted.stan.dso, file = "probit.unrestricted.stan.dso.RData")
} else{
  load("probit.unrestricted.stan.singlefree.dso.RData")
  
  model.data = list(N = N, k = ncol(X),
                    y = c(y), X = X,
                    mu_beta = mu.beta, sd_beta = sd.beta)
  stan.fit = stan("stan_probit_model_singlefree.stan",
                  data = model.data,
                  chains = 1,
                  fit = unrestricted.stan.dso,
                  warmup = burn.iter, iter = burn.iter + sample.iter)
  ...
  # unrestricted.stan.dso = stan.fit
  # save(unrestricted.stan.dso, file = "probit.unrestricted.stan.singlefree.dso.RData")
}
...
#Parameter estimates
beta.star = apply(as.matrix(stan.extract$beta), 2, mean)
...
if (num.runs > 1){
  ...
  kde.result = akj(stan.extract$beta, z = beta.star[parameters])
  log.density.unconditional = log(kde.result$dens)
}
...
if (num.runs > 1){
  ...
  for (run in 1:(num.runs - 1)){
    ...
    if (num.free.params == 1){
      stan.file = "stan_probit_model_iterative_singlefree.stan"
      load("probit.restricted.singlefree.stan.dso.RData")
    } else{
      stan.file = "stan_probit_model_iterative.stan"
      load("probit.restricted.stan.dso.RData")
    }
    ...
    # save(restricted.stan.dso, file = "probit.restricted.stan.dso.RData")
    # save(restricted.stan.dso, file = "probit.restricted.singlefree.stan.dso.RData")
...
log.lik = function(beta){
  z = X %*% beta
  prob = pnorm(z, lower.tail = TRUE, log = TRUE)
  prob.inv = pnorm(z, lower.tail = FALSE, log = TRUE)
