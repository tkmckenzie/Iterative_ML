...
if ("chib_model_9_results.RData" %in% list.files()){
  load("chib_model_9_results.RData")
} else{
  log.marginal.results = data.frame(logit = rep(NA, num.reps),
                                    probit = rep(NA, num.reps),
                                    model = "C + log(X.2) + X.3 + X.4 + X.5")
}
...
while (rep < num.reps){
  ...
  rep = sum(complete.cases(log.marginal.results))
  save(log.marginal.results, file = "chib_model_9_results.RData")
}
