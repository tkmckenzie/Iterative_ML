...
if (ncol(X) > 1){
  ...
  load("logit.unrestricted.stan.singlefree.dso.RData")
  
  model.data = list(N = N, k = ncol(X),
                    y = c(y), X = X,
                    mu_beta = mu.beta, sd_beta = sd.beta)
  stan.fit = stan("stan_logit_model_singlefree.stan",
                  data = model.data,
                  chains = 1,
                  fit = unrestricted.stan.dso,
                  warmup = burn.iter, iter = burn.iter + sample.iter)
  ...
  # unrestricted.stan.dso = stan.fit
  # save(unrestricted.stan.dso, file = "logit.unrestricted.stan.singlefree.dso.RData")
}
...
if (num.runs > 1){
  ...
  for (run in 1:(num.runs - 1)){
    ...
    # save(restricted.stan.dso, file = "logit.restricted.stan.dso.RData")
